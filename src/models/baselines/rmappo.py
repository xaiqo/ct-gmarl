...
class RMAPPOAgent(BaseAgent):
    ...
    def select_action(
        ...
        **kwargs
        ...
        log_prob = (torch.log(dist_type.gather(1, a_type.unsqueeze(-1)).squeeze(-1)) + 
    ...
    def evaluate_actions(
        ...
        **kwargs
        ...
        lp_type = torch.log(dist_type.gather(1, actions[:, 0].unsqueeze(-1)).squeeze(-1))
        lp_target = torch.log(dist_target.gather(1, actions[:, 1].unsqueeze(-1)).squeeze(-1))
        ...
        return (lp_type + lp_target).unsqueeze(-1), (ent_type + ent_target).unsqueeze(-1), {}
